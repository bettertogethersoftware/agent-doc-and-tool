...
- Use the Documents tab's **Default matching rules** as inherited folder defaults, then select any document folder to override its extension patterns, exact filenames, or filename case sensitivity. Tool folders expose the same **Folder document rules** in their Documents tab, so their nearby documentation and **Scan documents** results can use a different match set without changing other folders. Exact-file grants remain independent of matching rules; protected paths, configured secrets, ignore patterns, and search limits remain global.
...
`roots` are searched recursively. Root names must be unique case-insensitively within their source. Every `files` entry requires a human-readable `name` and exact `path`; exact-file names and paths must each be unique within their source, and exact grants do not need to match an extension or filename rule. `enabled` defaults to `true`, or set it to `false` to retain an entry without scanning or granting it. Path-only objects and string file entries are invalid. A disabled row deactivates that grant rather than creating a deny rule, so the same physical file can remain accessible when it is covered by another enabled parent folder. Source-level `extensions`, `fileNames`, and `caseSensitive` values are the defaults for every root without a `documentMatching` override. A root can set `documentMatching.mode` to `override` and provide `extensions`, `fileNames`, and `caseSensitive`; `mode: "inherit"` or an omitted object uses the source defaults. Extension values accept either an array of suffixes or a semicolon-separated string; `.json`, `*.json`, `**.json`, and `**/*.json` all normalize to the same `.json` suffix rule. `fileNames` contains exact names matched anywhere beneath that root. Folder case sensitivity controls filename and extension matching; content-query case sensitivity remains source-wide.
