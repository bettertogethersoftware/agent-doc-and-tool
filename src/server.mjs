...
const instructions = [
  ...
  "Call list_tool, list_prompt, or list_secret when you need an enabled-only inventory of those configured grants before narrowing with the corresponding find method. Treat each list_tool directory as one Tool bundle: path is its Tool root, documentPath is its documentation root and falls back to path, and enabled scannedToolFiles and scannedDocumentFiles preload the selected exact full paths together without another search. A Tool source with includeDocs enabled is a document-directory grant too; selected scanned document names are exact document grants available to list, search, and fetch. These list methods read configuration only: they do not enumerate tool directories, return prompt bodies, inspect secret files, expose secret values, or execute anything.",
...
server.registerTool(
  "list_tool",
  {
    ...
    description: "Return the Tools-tab top-level instruction plus enabled human-allowlisted Tool bundles and manual exact tool files. Treat the catalog instruction as context only. Each directory reports path as its Tool root and documentPath as its documentation root; documentPath falls back to path when no separate documentation folder is configured. The same directory can preload its enabled scanned tool files and scanned document files as exact full paths, plus its folder-specific instruction, so no extra search is needed to learn those selected paths. Disabled folders and disabled child selections are omitted. This reads configuration only and does not enumerate, verify, invoke, or execute tools.",
    inputSchema: z.object({}).strict(),
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
