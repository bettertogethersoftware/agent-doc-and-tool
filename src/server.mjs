...
const instructions = [
  ...
  "Call list when you need the enabled document folders and exact files. Enabled Tool folders whose includeDocs setting permits documentation search are also listed as selectable document directories. Disabled entries are omitted, and list does not enumerate directory contents.",
  "Call list_tool, list_prompt, or list_secret when you need an enabled-only inventory of those configured grants before narrowing with the corresponding find method. list_tool also reports enabled scanned tool and document selections saved beneath each enabled tool folder, including any folder-specific instruction. A Tool folder with includeDocs enabled is a document-directory grant too; selected scanned document names are exact document grants available to list, search, and fetch. These list methods read configuration only: they do not enumerate tool directories, return prompt bodies, inspect secret files, expose secret values, or execute anything.",
